...
__device__ __forceinline__ void null_asm() {
  asm volatile(";");
}
...
__global__ void gemm2_kernel(const float* A, const float* B, float* C, const int M, const int N, const int K) {
  //TODO 每个CTA应该处理多个block -> 需要两个循环
  //TODO Mtile & Ntile conflict
  ...
  const float* A_lds_ptr = (const float*)(
    Mtile + (K_TILE_SIZE - 1) * M_TILE_SIZE
    + (warp_id / N_WARP_SIZE) * M_THREAD * 8
    + (lane_id / N_THREAD) * 4
  );
  /*! \todo Zig-zag to avoid bank conflict */
  const float* B_lds_ptr = (const float*)(
    Ntile + (warp_id % N_WARP_SIZE) * N_THREAD * 8 + (lane_id % N_THREAD) * 4
  );
  ...
  for (int p = 0; p < K / K_TILE_SIZE; ++p) {

    {
      ...
      float4 a_tmp4;
      float4* A_sts_tmp4_ptr = (float4*)A_sts_ptr;
      a_tmp4.x = A_ldg_ptr[0];
      a_tmp4.y = A_ldg_ptr[K_TILE_SIZE];
      a_tmp4.z = A_ldg_ptr[K_TILE_SIZE * 2];
      a_tmp4.w = A_ldg_ptr[K_TILE_SIZE * 3];
      A_sts_tmp4_ptr[0] = a_tmp4;
      // load B to Mtile
      /*! \todo does new asm matters? lgd2s */
...
/*! \brief a block compute 128 * 128 tile */
__global__ void gemm2_debug_kernel(const float* A, const float* B, float* C, const int M, const int N, const int K, float* D = nullptr) {
  //* BUG FREE
  __shared__ __align__(M_TILE_SIZE * 4) float Mtile[M_TILE_SIZE * K_TILE_SIZE];
  __shared__ __align__(N_TILE_SIZE * 4) float Ntile[K_TILE_SIZE * N_TILE_SIZE];
  ...
  for (int p = 0; p < K / K_TILE_SIZE; ++p) {
...

    {
      ...
      float4* A_sts_tmp4_ptr = (float4*)A_sts_ptr;
      a_tmp4.x = A_ldg_ptr[0];
      a_tmp4.y = A_ldg_ptr[K];
      a_tmp4.z = A_ldg_ptr[K * 2];
      a_tmp4.w = A_ldg_ptr[K * 3];
      A_sts_tmp4_ptr[0] = a_tmp4;
      // load B to Mtile
      /*! \todo does new asm matters? lgd2s */
      B_sts_ptr[0]  = B_ldg_ptr[0];
  ...
  for (int m = 0; m < 4; ++m) {
    ...
    #else
    {
      #pragma unroll
      for (int n = 0; n < 4; ++n) {
        C_stg_ptr[n] = C_frag[m][n];
      }
      #pragma unroll
      for (int n = 4; n < 8; ++n) {
        C_stg_ptr[n + 32 - 4] = C_frag[m][n];
      }
    }
    #endif
...
float* gemm2() {
  ...
  gemm2_debug_kernel<<<gridDim, 256>>>(cuA, cuB, cuC, M, N, K);
